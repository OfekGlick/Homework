...
def kafka_stream():
    ...
    def handle_batch(batch_df: pyspark.sql.DataFrame, batch_id):
        ...
        batch_df = batch_df.filter(batch_df.Year_Rec >= 1950)
...
def groupby_batchid():
    ...
    ger_swe = ger_swe.withColumn("PRCP", F.col('PRCP_sum') / F.col('PRCP_count')) \
        .withColumn("TMAX_", F.col('TMAX_sum') / F.col('TMAX_count')) \
        .withColumn("TMIN", F.col('TMIN_sum') / F.col('TMIN_count')) \
    ...
    ger_swe.write \
        ...
        .mode("append") \
        .option("url", url) \
        .option("dbtable", "DataStore_Stage2") \
        .option("user", username) \
        .option("password", password) \
        .save()
